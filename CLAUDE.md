...
- HA app packaging at the **repo root** — `config.yaml` (manifest with ingress + panel), `Dockerfile` (multi-stage build with build context = repo root, so it can pull in the `server/` and `display/` workspaces), `run.sh` (entrypoint), `build.yaml`, `DOCS.md` / `CHANGELOG.md` / `translations/en.yaml`. The whole repo IS the add-on (single-app layout, no `repository.yaml`). Users install it by adding `https://github.com/qrobinso/cosmos-ha-dashboard` to **Settings → Apps → Repositories**. The app auto-discovers the Supervisor token + MQTT broker.
