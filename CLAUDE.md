...
- `canvas` widget — sandboxed iframe (`sandbox="allow-scripts"`) running user/agent-authored HTML/CSS/JS. Templates inside the content (`{{ states("...") }}`) are rendered server-side by HA via the `render_template` WS subscription (pin-for-pin HA-compatible). The iframe gets a small read-only postMessage bridge exposing `window.cosmos.{entity, subscribe, size, scene, font, tokens, ready}` plus CSS variables (`--cosmos-font-family`, `--cosmos-font-scale`, `--cosmos-bg`, `--cosmos-fg`, `--cosmos-w/h`). See `docs/canvas-widget.md` (user) and `docs/canvas-widget-agent.md` (LLM contract).
