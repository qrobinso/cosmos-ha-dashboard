...
- Music video candidate selection applies **two hard requirements** before ranking (`musicvideo/ytdlp.ts`): the title must contain both `official` and `video`, and the channel must be the artist's own (`isArtistChannel` — VEVO and `…music` suffixes fall out of normalization; `channel_is_verified` is deliberately NOT sufficient, since lyric/re-upload channels like `7clouds` are verified too). Survivors are then ranked and must clear `MIN_SCORE`. This is a deliberate accuracy-over-coverage trade with a measured cost: across 12 well-known songs, 7 play and 5 show nothing — 4 of those have famous official videos whose titles simply lack the wording (`Kendrick Lamar - HUMBLE.`, `Radiohead - Karma Police`, `Taylor Swift - Blank Space`, `Billie Eilish - bad guy`). Relaxing the title requirement means restoring the Art Track rule described in `score.ts`, because YouTube's auto-generated audio uploads beat the real video on every other signal. Retune only against the two fixtures in `musicvideo.score.test.ts`.
