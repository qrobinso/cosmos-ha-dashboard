...
- Music video scoring gates on a confidence threshold (`MIN_SCORE` in `musicvideo/score.ts`): if the best of five candidates falls below it, **nothing plays** and the reason is logged with a per-signal breakdown. Strongest positive is `official` + `video` in the title (matched as two words anywhere, since orderings vary). Strongest negative is the **Art Track** rule — a title that is *exactly* the song name on the artist's own channel is YouTube's auto-generated audio upload, not a video. That rule keys on the bare title rather than on missing "official video" wording, because many real official videos (Kendrick's "HUMBLE.", Radiohead's "Karma Police") carry no such wording and were rejected outright when it did. Duration is weak and asymmetric — much-longer is penalized (medleys, compilations), shorter barely at all (official videos are often single edits, and some run long on intros). Retune only against the two fixtures in `musicvideo.score.test.ts`; these weights trade off and have broken each other twice.
