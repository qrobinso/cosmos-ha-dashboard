...
- Widget layering — every widget kind honours an optional integer `config.layer` (`display/src/lib/scene/WidgetSlot.svelte`): negative sits behind overlapping siblings, positive in front, absent/0 keeps the previous implicit DOM-order stacking. Set in the editor under Placement → Layer. The music video widget also takes `config.opacity` (0..1) and `config.edge_fade` (px, 0 = off), so it can run as a faded, soft-edged full-bleed backdrop with the rest of the scene layered on top. The edge fade reuses `WidgetSlot`'s masked-gradient technique (which is disabled for this kind) with a user-settable distance; the mask is emitted only when non-zero, is static, and sits on an already-composited `<video>` layer, so it is GPU-applied rather than repainting frames.
