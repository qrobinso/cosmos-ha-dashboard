...
- `musicvideo/` (server) — Resolves the YouTube music video for a `media_player`'s current track. `trackKey.ts` normalizes `artist|title` (stripping `feat.` / remaster decoration) into a cache key; `ytdlp.ts` is the only file that knows yt-dlp exists, behind a `VideoLookup` interface (swap in a YouTube Data API client by writing one new implementation); it runs a two-phase lookup — a fast `--flat-playlist ytsearch5` for candidate metadata, then the same format selector to resolve a stream for the winner only. `score.ts` is a pure scorer (duration vs HA's `media_duration`, artist/channel match, verified channel, official-video bonus, penalties for live/lyric/remix/cover/karaoke) with a `MIN_SCORE` confidence gate below which nothing plays; `cache.ts` is a two-tier SQLite cache (durable `trackKey → videoId`, ephemeral 4h `videoId → streamUrl`, 24h negative results); `resolver.ts` mirrors `createCanvasResolver`'s lifecycle and **never blocks the scene push** — a cache miss returns `null` immediately and fires `onUpdate` → `markDisplayDirty` → re-push once the lookup lands. `fileStore.ts` keeps downloaded copies on disk (`music_video_file`, migration v14) under a user-set MB cap, evicting least-played-first then oldest-of-those; the stream route serves a local file when present, otherwise proxies and kicks off a background download, so the first play is instant and every later one never leaves the house. The kiosk plays from `/api/musicvideo/stream/:videoId`, never a raw googlevideo URL. `overrides.ts` is a separate, durable `music_video_override` table (never pruned) holding a user's manual pin or block per track key; `resolver.ts` consults it *ahead of* the cache, cooldowns, and concurrency cap, so a pin or block always wins over automatic resolution. `youtubeUrl.ts` parses whatever a user pastes (watch/youtu.be/music/shorts/embed/mobile URLs, or a bare id) into a video id.
- Widget layering — every widget kind honours an optional integer `config.layer` (`display/src/lib/scene/WidgetSlot.svelte`): negative sits behind overlapping siblings, positive in front, absent/0 keeps the previous implicit DOM-order stacking. Set in the editor under Placement → Layer. The video backdrop widget also takes `config.opacity` (0..1), `config.edge_fade` (px, 0 = off) and `config.fade_ms` (track-change crossfade, default 800), so it can run as a faded, soft-edged full-bleed backdrop with the rest of the scene layered on top. The edge fade reuses `WidgetSlot`'s masked-gradient technique (which is disabled for this kind) with a user-settable distance; the mask is emitted only when non-zero, is static, and sits on an already-composited `<video>` layer, so it is GPU-applied rather than repainting frames. On a track change the widget fades the old video out, swaps the source, then fades back in only on `canplay` — `shownId` lags `video_id` for exactly this reason, so the fade-in never reveals a blank or stuttering element.
...
- `music_video_cache` / `music_video_stream` are pruned at boot and hourly (`cache.prune()` in `musicvideo/cache.ts`, wired in `index.ts`): expired negatives and stale stream URLs go, and the durable lookup table is capped at `MAX_CACHE_ROWS` (5000), oldest-first. Positive `trackKey → videoId` rows never expire on their own — a videoId stays correct forever — so the cap is their only bound.
- Video backdrop lookups are skipped for non-music `media_content_type` values (`tvshow`, `episode`, `movie`, `video`, `game`, `app`, `url`) via a denylist in `assembler.ts`, since a TV episode reports title/artist too. Integrations reporting no content type still get a lookup. If a player reports an unusual type for music, add it to the allowed side.
- Music video candidate selection applies **two hard requirements** before ranking (`musicvideo/ytdlp.ts`): the title must contain both an `official` word and a `video` word — **in any of several languages** (`official/oficial/officiel/offiziell/ufficiale` and `video/videoclip/musikvideo/clip/clipe`, matched against a diacritic-folded title) — and the channel must be the artist's own (`isArtistChannel` — VEVO and `…music` suffixes fall out of normalization; `channel_is_verified` is deliberately NOT sufficient, since lyric/re-upload channels like `7clouds` are verified too). Survivors are then ranked and must clear `MIN_SCORE`. This is a deliberate accuracy-over-coverage trade with a measured cost: across 12 well-known songs, 7 play and 5 show nothing — 4 of those have famous official videos whose titles simply lack the wording (`Kendrick Lamar - HUMBLE.`, `Radiohead - Karma Police`, `Taylor Swift - Blank Space`, `Billie Eilish - bad guy`). Relaxing the title requirement means restoring the Art Track rule described in `score.ts`, because YouTube's auto-generated audio uploads beat the real video on every other signal. The multilingual spellings are not cosmetic: matching only English silently discarded EVERY Spanish-language official video, since artists label them "Video Oficial" — found in a real library where Bad Bunny's own videos on his own channel were rejected. `TITLE_PENALTIES` therefore also carries the non-English disqualifiers (`letra`, `en vivo`, `ao vivo`, `paroles`, …); without them, opening the gate would let a Spanish lyric video through instead. Retune only against the fixtures in `musicvideo.score.test.ts`.
...
- **YouTube is retiring progressive (muxed) formats.** Format 18 — which this feature was originally pinned to, because it plays in a bare `<video>` — is no longer offered for many videos, and where an older yt-dlp still reports it the URL 403s. `FORMAT` in `ytdlp.ts` is now a preference chain of **video-only** H.264 MP4s capped at 480p. Dropping audio is free here because the widget is always muted (sound comes from the HA speaker), so there is still no ffmpeg in the image. If playback breaks again, check whether the chain still resolves before suspecting scoring.
- Downloaded videos live at `VIDEO_CACHE_DIR` (`/share/cosmos/video-cache` in the add-on), deliberately NOT under `/data`: Home Assistant includes an add-on's `/data` in its backups, so caching video there would inflate every snapshot. The size cap is a Cosmos setting (`musicvideo.max_cache_mb`), not an add-on option — one control, on the Video Backdrop page.
- Play counts come from stream requests with no `Range` or a range starting at 0, which approximates "a playback started". A browser issues many ranged requests per play, so counting all of them would rank long videos above popular ones.
- `yt-dlp` is against YouTube's ToS and its extractors break when YouTube changes; a broken extractor degrades to a permanently hidden widget with no user-visible explanation. Requires occasional `yt-dlp -U` (the app image installs yt-dlp from PyPI against the image's own Python — the standalone musllinux build was tried and fails on this Alpine base, whose musl lacks the `pwritev2` symbol Python 3.14 needs; in-place updates are `pip3 install -U --break-system-packages yt-dlp`, not `yt-dlp -U`). `YTDLP_VERSION` in `addon/Dockerfile` is now EMPTY by default — the image installs the newest yt-dlp, and an `ADD` of GitHub's yt-dlp latest-release endpoint busts that layer's Docker cache whenever upstream publishes, so "latest" is not silently frozen at whenever the layer was first built. Set `YTDLP_VERSION` to pin only as an escape hatch from a broken upstream release. Note this refreshes on **add-on update**, not continuously: an install left alone for months still ages. Deliberate, accepted tradeoff — see `docs/superpowers/specs/2026-08-02-music-video-widget-design.md`.
- The video backdrop widget's position sync wraps `position % videoDuration`, so a video shorter than the track restarts at an arbitrary offset rather than at a musically sensible point.
...
- The overrides admin page polls `/api/musicvideo/now-playing` every 5s rather than riding the existing WS push. Fine for a briefly-open admin tab; move it onto the socket if people start leaving it open all day.
- `music_video_override.created_at` is reset on every re-pin, so the admin list's "newest first" is really "most recently changed first". Fine for the list's purpose; note it if a stable creation date is ever wanted.
- A lookup already in flight when an override is set still fires one stale `onUpdate`. It self-heals because setting an override marks all displays dirty and forces an immediate re-push. Untested.
- **The server suite exits 139 (SIGSEGV) on most runs, with every test passing.** Now diagnosed rather than merely known: it is a teardown race, not a test failure. `test/ws.scene-push.test.ts` leaves `@hono/node-server` holding a socket-close timer that fires after the worker has begun shutting down, throwing `socket.destroySoon is not a function`; the crash follows. Run `npx --workspace server vitest run --pool=forks --poolOptions.forks.singleFork=true` for a clean signal — it reports the real pass/fail (still nonzero on the same teardown error, but with a full summary). The fix is to await the WS test's server close properly; nobody has yet.
- `npm --workspace server run build` (tsc) does NOT type-check `server/test/`, so an interface change will not surface broken test fakes via the compiler — grep for them instead.
- The overrides page's block action reports errors via `window.alert` rather than the page's inline error slot.
- The overrides admin page has not been hand-verified in a browser.
