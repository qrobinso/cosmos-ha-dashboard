...
- Music video candidate scoring gates on a confidence threshold (`MIN_SCORE` in `musicvideo/score.ts`): if the best of five candidates scores below it, **nothing plays** and the reason is logged via `console.error` with a per-signal breakdown. Duration is deliberately a weak, asymmetric signal — much-longer-than-track is penalized (medleys, compilations), shorter is barely penalized (official videos are often single edits) — so an explicit "official video" title outranks a coincidental runtime match. Two real fixtures in `musicvideo.score.test.ts` (Solange "Weary", Bowie "Heroes") are the regression guard; retuning by feel has already broken one while fixing the other.
