...
- **Widget editor** (`src/lib/admin/`) — the scene editor's widget surface, a self-contained three-pane (rail / canvas / inspector) module:
  ...
  - `WidgetInspector.svelte` — header (icon · editable `config.name` input · kind tag · Duplicate · two-step Delete), the per-kind field group, a shared Surface section (transparent / corner radius), a Placement section (col/row/w/h two-way bound to `position`), and an empty-state variant. (Kind isn't re-typable in place — delete + add from the palette instead.)
...
- `src/routes/admin/` — admin editor pages, all wrapped in the `.cosmos-admin` shell:
  ...
  - `designs/+page.svelte`, `designs/new/+page.svelte`, `designs/[slug]/+page.svelte` — design-system management. List renders each pack as a typographic specimen poster in its own palette + font; built-ins are read-only with a "Duplicate as user pack" affordance; user packs have an inline editable name + Save/Delete. Edit page is two-column (≥960px): source editor on the left, sticky live preview on the right (frontmatter parsed client-side for instant feedback, body rendered with a tiny inline markdown renderer).
  ...
  - `musicvideo/+page.svelte` — Music Video overrides page: pick the watched `media_player`, see what it's playing and how it currently resolves (polls `GET /api/musicvideo/now-playing` every 5s), pin a pasted YouTube link or block a song entirely, and browse/edit existing overrides plus the last 50 automatic resolutions.
