...
- `src/aerials/` — Apple TV aerial backgrounds. `catalog.ts` holds the hardcoded tvOS bundle URLs (newest first) and `parseEntries` (reduces Apple's `entries.json` to the 1080p H.264 clips a Chromium kiosk can play, in four stable categories); `untar.ts` is a 40-line ustar reader so the bundle needs no dependency; `store.ts` persists the parsed catalog as one JSON row in `settings`; `refresh.ts` refreshes it weekly with a single in-flight fetch; `select.ts` expands a scene's `{ids, categories}` into the ordered playlist; `download.ts` fetches clips to disk in the background (`.part` then rename). `src/api/aerials.ts` exposes the catalog, refresh, storage cap, and the `stream/:id` proxy-then-cache route; `api/localFile.ts` is the Range-aware local serving helper shared with music videos.
