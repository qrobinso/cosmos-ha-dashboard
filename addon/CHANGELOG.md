...
- Feat: **Video Backdrop overrides.** New **Video Backdrop** page in the sidebar lets you pin a specific YouTube video to a song, or block a song from ever playing one — by hand, per song. Pick the media_player Cosmos should watch, and the page shows what's playing right now and how it currently resolves. Paste a YouTube link (watch, `youtu.be`, `music.youtube.com`, or Shorts) and save — Cosmos checks the link actually plays before storing it. A pin or block is permanent and always wins over the automatic matcher, on every display, until you remove it. Also lists recent automatic resolutions (including misses) with search across every song Cosmos remembers and paging, so you can fix one you heard days ago. When nothing matched, the page now explains **why** — which candidates it saw and what disqualified them. See `docs/video-backdrop-overrides.md`.
- Change: **the admin menu is reorganised.** Three destinations — **Scenes**, **Displays**, **Settings** — with Designs and Video Backdrop as tabs under Scenes. The agent is now a slide-over you can open from any page instead of a separate page, and Docs is an icon in the top bar.
- Feat: **Now Playing on the Overview page.** Whenever anything is playing on any media player, a card appears above your displays showing the track, linking straight to the Video Backdrop page for it.
